...
// ?
	...
	function hasClasse(markup,cssClass){
		
			var regex = new RegExp('class=["\'](?:[\\w\\s]+)?'+ cssClass + '(?:[\\w\\s]+)?["\']');
		
			return regex.test(markup); // retorna true caso dê match 

		}


	// uso do foreach para exibição da mensagem no console
	var classes = ['container', 'text', 'date', 'excerpt', 'main'];

	classes.forEach(function(item){
		console.log(hasClasse(markup, item) + ' para a classe '+ item);
	});
